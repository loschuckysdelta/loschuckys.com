...
const FRONTENDS=(process.env.FRONTEND_URL||"*").split(",").map(x=>x.trim().replace(/\/$/,"")).filter(Boolean);
...
app.use(cors({
  origin(origin,cb){
    if(!origin || FRONTENDS.includes("*") || FRONTENDS.includes(origin.replace(/\/$/,""))) return cb(null,true);
